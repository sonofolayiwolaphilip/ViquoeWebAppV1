...
export default function SignUpPage() {
  ...
  // Extract query parameters passed from CreateRfqForm
  const redirectTarget = searchParams.get("redirect") || "/dashboard/rfqs";
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data, error: authError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          // Pass extra application context directly to Postgres database trigger
          data: {
            company_name: companyName,
            phone_number: phoneNumber,
            role: role,
          },
          // Ensures post-confirmation email links bring the user back to the sync route
      ...
      if (data?.session) {
        // Immediate session created (Email Confirmation Disabled) -> Route directly to sync
        router.push(redirectTarget);
        router.refresh();
      } else if (data?.user) {
        // Email Confirmation Enabled -> Direct user to login with redirect preserved
      ...
      <div className="max-w-md w-full bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Register your institutional business profile
          </p>
        </div>

        {/* Dynamic Context Notification */}
        {intent === "publish_rfq" && (
          <div className="p-3 bg-blue-50 border border-blue-200 text-blue-800 text-xs font-medium rounded-lg mb-4">
            ℹ️ Your RFQ draft is saved locally. Register your account to finalize and publish it.
          </div>
        )}
